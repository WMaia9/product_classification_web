...
if btn_predict:
  ...
  st.subheader(f'Categoria: {label_categoria[np.argsort(pred[1].flatten())[::-1]][:3]}')
  st.subheader(f'Subcategoria: {label_subcategoria[np.argsort(pred[2].flatten())[::-1]][:5]}')
  st.subheader(f'Produto: {label_produto[np.argsort(pred[3].flatten())[::-1]][:5]}')